...
\subsection{Implementing Tables and Integrity Constraints}
This section presents the physical implementation of the relational schema in MySQL. 
The detailed relations, primary keys, and foreign keys were already described in the Mapping section. 
Therefore, this section focuses on how the mapped relations were translated into executable SQL table definitions and integrity constraints.
The database is implemented under the name \texttt{steam}. 
The implementation includes entity tables, weak entity tables, relationship tables, and associative tables required by the Steam Store Management system.


\subsubsection{Full SQL Implementation}

The following SQL script shows the complete table creation implementation used in the project.

\begin{lstlisting}[language=SQL, caption={Full SQL script for table creation and integrity constraints}, label={lst:create-tables}]
CREATE TABLE `USER` (
...
After executing the schema script, all tables were successfully created in the \texttt{steam\_store} database. 
The created schema reflects the relational mapping from the previous design phase while enforcing important integrity constraints directly at the database level.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{graphics/create_table_result.png}
    \caption{Successful creation of database tables in MySQL Workbench}
    \label{fig:create-table-result}
\end{figure}